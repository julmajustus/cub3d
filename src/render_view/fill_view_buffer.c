...
static void	fill_wall_colors(t_caster *c, int x, int y)
{
	uint32_t	color;
	uint8_t		*pixels;
	int		pixel_pos;
...
void	fill_view_buffer(t_caster *c)
{
	...
	while (++x < WIDTH)
	{
		...
		fill_wall_colors(c, x, c->draw_start[x]);
		if (FC_TEXTURES)
			fill_fc_texture_colors(c, c->draw_end[x], x);
		else
		 	fill_fc_plain_colors(c, c->draw_end[x], x);
