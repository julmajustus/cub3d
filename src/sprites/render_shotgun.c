...
/*   Updated: 2024/11/13 03:55:54 by jmakkone         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_gun_pixel(t_caster *c, int screen_x, int screen_y, int sprite_size)
{
	int		y;
	int		x;
	double	step;
	...
	while (++y < sprite_size)
	{
		...
		while (++x < sprite_size)
		{
			...
			c->gun->tex_index = c->gun->frame_offset + (c->gun->tex_y * 64 + c->gun->tex_x) * 4;
			c->gun->color = (c->gun->sprite_texture->pixels[c->gun->tex_index] << 24) |
				(c->gun->sprite_texture->pixels[c->gun->tex_index + 1] << 16) |
				(c->gun->sprite_texture->pixels[c->gun->tex_index + 2] << 8) |
				c->gun->sprite_texture->pixels[c->gun->tex_index + 3];
			if (screen_x + x >= 0 && screen_x + x < WIDTH &&
				screen_y + y >= 0 && screen_y + y < HEIGHT)
				mlx_put_pixel(c->window->gun, screen_x + x, screen_y + y, c->gun->color);
		}
	}
}
